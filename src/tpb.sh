...
tpb_parse_xml () {
    ...
    for line in $magnets; do
        ...
        title=$(url_encode_string url-to-text "$title" | sed -E 's/[+.]/ /g')
        id=$(echo "$line" | egrep -o 'magnet:\?xt=urn:btih:[0-9A-Z]{,}' | cut -d':' -f4)
        size=$(echo "$xml_result" | grep "$id" | egrep -o '[0-9.]{,}&nbsp;(M|G)iB' | sed 's/&nbsp;/ /g')
        echo -e "
${SHELL_STDOUT_CODE} $iter ${ECHO_RESET}|${SHELL_HOST_PROMPT_CODE} $size | $title ${ECHO_RESET}
