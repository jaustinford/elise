...
print_message () {
    ...
    if [ "$1" == 'stdout' ]; then
        ...
        if [ ! -z "$3" ]; then
            echo -e " [ ${SHELL_STDERR_CODE}ERROR${ECHO_RESET} - ${SHELL_STDERR_CODE}$(date '+%Y.%m.%d-%H:%M:%S')${ECHO_RESET} ] ${SHELL_HOST_PROMPT_CODE}$key${ECHO_RESET} | ${SHELL_CWD_PROMPT_CODE}$3${ECHO_RESET}"

        else
            echo -e " [ ${SHELL_STDERR_CODE}ERROR${ECHO_RESET} - ${SHELL_STDERR_CODE}$(date '+%Y.%m.%d-%H:%M:%S')${ECHO_RESET} ] ${SHELL_HOST_PROMPT_CODE}$key${ECHO_RESET}"
